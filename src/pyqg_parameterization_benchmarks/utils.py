...
def diagnostic_differences(ds1, ds2, T=10):
    ...
    distribution_quantities = dict(
        ...
        u='ufull',
        v='vfull',
        KE='add(pow(ufull,2),pow(vfull,2))',
        Ens='pow(curl(ufull,vfull),2)',
    )
    ...
    for label, expr in distribution_quantities.items():
        for z in [0,1]:
            q1 = FeatureExtractor(ds1.isel(lev=z,time=slice(-T,None)))(expr).ravel()
            q2 = FeatureExtractor(ds2.isel(lev=z,time=slice(-T,None)))(expr).ravel()
    ...
    def spectral_rmse(spec1, spec2):
        # Initialize pyqg models just so we can use pyqg's calc_ispec helper
        ...
        # Take error over common wavenumbers
